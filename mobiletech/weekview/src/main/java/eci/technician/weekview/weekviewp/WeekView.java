...
import static eci.technician.weekview.weekviewp.WeekViewUtil.isSameDay;
import static eci.technician.weekview.weekviewp.WeekViewUtil.today;
...
import eci.technician.weekview.R;
import kotlin.Pair;
 ...
 * and modified some code for Alvaro Saavedra for MT app
 */
public class WeekView extends View {
    ...
    //clock painters
    private Paint mClockInPaint;
    private Paint mLineClockInPaint;
    private Paint mClockOutPaint;
    private Paint mLineClockOutPaint;
    ...
    private List<WeekViewEvent> mClockEvents;
    ...
    private List<? extends WeekViewEvent> mPreviousClockPeriodEvents;
    private List<? extends WeekViewEvent> mCurrentClockPeriodEvents;
    private List<? extends WeekViewEvent> mNextClockPeriodEvents;
...
//            }
    ...
    private void init() {
        ...
        // Prepare ClockIn and ClockOut line color Paint
        mClockInPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mClockInPaint.setColor(Color.GREEN);
        mLineClockInPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mLineClockInPaint.setStrokeWidth(8);
        mLineClockInPaint.setColor(Color.GREEN);


        mClockOutPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mClockOutPaint.setColor(Color.RED);
        mLineClockOutPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mLineClockOutPaint.setStrokeWidth(8);
        mLineClockOutPaint.setColor(Color.RED);

        mBackgroundTextPaint = new Paint();
        mBackgroundTextPaint.setColor(Color.RED);
        mBackgroundTextPaint.setStyle(Paint.Style.FILL);

        mClockTextPaint = new Paint();
        mClockTextPaint.setColor(Color.BLACK);
        mClockTextPaint.setTextSize(40);
        ...
        mDefaultEventColor = Color.parseColor("#E69fc6e7");
...
//        }
    ...
    /**
     * Draws the clock event as a line, similar to the current Time line, with custom colors
     * @param canvas Canvas to draw
     * @param startPixel A reference to the first pixel to draw
     * @param startY A reference to the pos Y to draw
     * @param wid Space for the time
     * @param date Date to know in Which day draw
     * @param clockEvent The event that contains the
     */
    private void drawClockLine(Canvas canvas, float startPixel, float startY, float wid, Calendar date, WeekViewEvent clockEvent) {
        EventType eventType = clockEvent.getEventType();
        mClockInPaint.setColor(clockEvent.getColor());
        mLineClockInPaint.setColor(clockEvent.getColor());
        mClockOutPaint.setColor(clockEvent.getColor());
        mLineClockOutPaint.setColor(clockEvent.getColor());
        mClockTextPaint.setColor(clockEvent.getColor());
        float beforeNow = (date.get(Calendar.HOUR_OF_DAY) + date.get(Calendar.MINUTE) / 60.0f) * mHourHeight;
        canvasclipRect(canvas, 0, mHeaderHeight + mHeaderRowPadding * 3 - 70, getWidth(), getHeight(), Region.Op.REPLACE);
        canvas.drawRoundRect(startPixel - 10, startY + beforeNow - 10, startPixel + 10, startY + beforeNow + 10, 10, 10, eventType == EventType.CLOCK_IN ? mClockInPaint : mClockOutPaint);
        canvas.drawLine(startPixel, startY + beforeNow, startPixel + wid - mHeaderColumnWidth, startY + beforeNow, eventType == EventType.CLOCK_IN ? mLineClockInPaint : mLineClockOutPaint);

        String textToDraw = clockEvent.getName();
        canvas.drawText(textToDraw, startPixel, startY + beforeNow - 6, mClockTextPaint);
        canvasclipRect(canvas, 0, 0, getWidth(), getHeight(), Region.Op.REPLACE);
    }
    ...
    private void drawEvents(Calendar date, float startFromPixel, Canvas canvas) {
        ...
        /**
         * Draw the clock events after Drawing the events
         * Verifying to draw in the correct day
         */
        if (mClockEvents != null && !mClockEvents.isEmpty()) {
            float startY = mHeaderHeight + mHeaderRowPadding * 3 + mTimeTextHeight / 2 + mHeaderMarginBottom + mCurrentOrigin.y;
            float wid = mWidthPerDay;
            for (WeekViewEvent clockEvent : mClockEvents) {
                if (isSameDay(clockEvent.getStartTime(), date)) {
                    drawClockLine(canvas, startFromPixel, startY, wid, clockEvent.getStartTime(), clockEvent);
                }
            }
        }
    }
    ...
    private void getMoreEvents(Calendar day) {
        ...
        if (mClockEvents == null)
            mClockEvents = new ArrayList<>();
        ...
        if (mRefreshEvents) {
            ...
            mPreviousClockPeriodEvents = null;
            mCurrentClockPeriodEvents = null;
            mNextClockPeriodEvents = null;
            mFetchedPeriod = -1;
        }

        /**
         * Add same logic (list of events)  to clock list of events
         */
        if (mWeekViewLoader != null) {
            ...
            if (!isInEditMode() && (mFetchedPeriod < 0 || mFetchedPeriod != periodToFetch || mRefreshEvents)) {
                ...
                List<? extends WeekViewEvent> previousClockPeriodEvents = null;
                List<? extends WeekViewEvent> currentClockPeriodEvents = null;
                List<? extends WeekViewEvent> nextClockPeriodEvents = null;
                ...
                if (mPreviousPeriodEvents != null && mCurrentPeriodEvents != null && mNextPeriodEvents != null) {
                    if (periodToFetch == mFetchedPeriod - 1) {
                        ...
                        //copy login for clock events
                        currentClockPeriodEvents = mPreviousClockPeriodEvents;
                        nextClockPeriodEvents = mCurrentPeriodEvents;
                        ...
                        //copy login for clock events
                        previousClockPeriodEvents = mPreviousClockPeriodEvents;
                        currentClockPeriodEvents = mCurrentClockPeriodEvents;
                        nextClockPeriodEvents = mNextClockPeriodEvents;
                    } else if (periodToFetch == mFetchedPeriod + 1) {
                        previousPeriodEvents = mCurrentPeriodEvents;
                        currentPeriodEvents = mNextPeriodEvents;
                        //copy login for clock events
                        previousClockPeriodEvents = mCurrentClockPeriodEvents;
                        currentClockPeriodEvents = mCurrentClockPeriodEvents;
                    }
                }

                if (currentPeriodEvents == null) {
                    List<? extends WeekViewEvent> loaded = mWeekViewLoader.onLoad(periodToFetch);
                    Pair<List<WeekViewEvent>, List<WeekViewEvent>> pair = EventUtil.INSTANCE.splitEvents(loaded);
                    currentPeriodEvents = pair.getFirst();
                    //copy login for clock events
                    currentClockPeriodEvents = pair.getSecond();

                }
                if (previousPeriodEvents == null) {
                    List<? extends WeekViewEvent> loaded = mWeekViewLoader.onLoad(periodToFetch - 1);
                    Pair<List<WeekViewEvent>, List<WeekViewEvent>> pair = EventUtil.INSTANCE.splitEvents(loaded);
                    previousPeriodEvents = pair.getFirst();
                    //copy login for clock events
                    previousClockPeriodEvents = pair.getSecond();
                }
                if (nextPeriodEvents == null) {
                    List<? extends WeekViewEvent> loaded = mWeekViewLoader.onLoad(periodToFetch + 1);
                    Pair<List<WeekViewEvent>, List<WeekViewEvent>> pair = EventUtil.INSTANCE.splitEvents(loaded);
                    nextPeriodEvents = pair.getFirst();
                    //copy login for clock events
                    nextClockPeriodEvents = pair.getSecond();
                }
                ...
                mEventRects.clear();
                mClockEvents.clear();
                sortAndCacheEvents(previousPeriodEvents, previousClockPeriodEvents);
                sortAndCacheEvents(currentPeriodEvents, currentClockPeriodEvents);
                sortAndCacheEvents(nextPeriodEvents, nextClockPeriodEvents);
                calculateHeaderHeight();
     ...
     * Cache the event for smooth scrolling functionality.
     *
     * @param event The event to cache. CLOCK EVENTS
     */
    private void cacheClockEvent(WeekViewEvent event) {
        if (event.getStartTime().compareTo(event.getEndTime()) >= 0)
            return;
        mClockEvents.add(event);
    }
     ...
     */
    private void sortAndCacheEvents(List<? extends WeekViewEvent> events, List<? extends WeekViewEvent> clockEvents) {
        sortEvents(events);
        sortEvents(clockEvents);
        for (WeekViewEvent event : events) {
            cacheEvent(event);
        }
        for (WeekViewEvent event : clockEvents) {
            cacheClockEvent(event);
        }
    }
